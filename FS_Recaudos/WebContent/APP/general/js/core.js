 ...
 * Archivo - $Id$
 * 
 * Número de versión - $Revision$ Ultima actualización - $Date$ Actualizado por - $Author$
 * 
...
function osm_validarCorreo(correo) {
	...
	// Si fallá la validación por jsonrpx se hace por expresión regular
 ...
 * 
 * Los caracteres especiales validados son: punto, apóstrofe, guión y espacio
 */
function osm_validarCadenaCaracteres(cadena, numeros, caracteresEspeciales) {
	var realfa = /^[\w]+$/;
	var realfanumerico = /^[A-Za-z0-9]+$/;
	var realfanumSpecials = /^[A-Za-z0-9áéíóúÁÉÍÓÚñÑ\.\'\- ]+$/;
	var realfaSpecials = /^[A-Za-záéíóúÁÉÍÓÚñÑ\.\'\- ]+$/;
...
/**
 * Limpia los espacios en blanco al final y al principio. Si es null retorna cadena vacía.
 * 
 * @param cadena
 * @returns
 */
...
function osm_formatoMoneDeci(numero){ 
	...
	if( caract !=',' ){
    	...
    	if(decimales!==undefined){
    		// Redondeamos al número de decimales
